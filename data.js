...
console.log(emojiList[2].aliases)

init()
function init(){
    text.addEventListener("keyup",search_result)
    search_result()
}
function search_result(){
    ...
    data = data.filter((search)=>{
        return search.aliases[0].includes(text.value)
    })
    let html=''
    for(let i=0; i<data.length; i++){
        html += ` <div id="new_div">
        <span id="icon">${data[i].emoji}</span>
        <span id="nick_name">${data[i].aliases}</span>
        <span id="description" style ="color: rgba(255, 255, 255,0.75);">${data[i].description}</span>
               </div><p class="line"></p>`
    }
